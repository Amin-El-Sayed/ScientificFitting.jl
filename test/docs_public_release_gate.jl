...
@testset "Public documentation release hygiene" begin
    ...
    @testset "First-user path is executable and honest" begin
        ...
        @test occursin("Julia 1.10", install) && occursin("latest stable Julia", install)
        @test occursin("NativeMinuit adapter requires Julia 1.11+", install)
        @test occursin("Pkg.add(\"ScientificFitting\")", install)
        @test !occursin("Pkg.add(url=", install)
