...
ax = Axis(
    fig[1, 1];
    title=L"\mathrm{Photoelectric\ threshold\ from\ two\ fitted\ regimes}",
    xlabel=L"\nu\ \mathrm{(THz)}",
    ylabel=L"U_0\ \mathrm{(V)}",
)
errorbars!(ax, frequency_THz, voltage_V, sigma_voltage_V; color=(:gray35, 0.55))
errorbars!(ax, frequency_THz, voltage_V, sigma_frequency_THz; direction=:x, color=(:gray35, 0.40))
scatter!(ax, frequency_THz[baseline_mask], voltage_V[baseline_mask]; color="#b85c38", marker=:diamond, label="baseline")
scatter!(ax, frequency_THz[emission_mask], voltage_V[emission_mask]; color="#14151a", label="emission")
...
vspan!(ax, threshold_THz - sigma_threshold_THz, threshold_THz + sigma_threshold_THz; color=("#7a5c00", 0.14), label="threshold 1σ")
vlines!(ax, [threshold_THz]; color="#7a5c00", linestyle=:dash, linewidth=2)
scatter!(ax, [threshold_THz], [line_model([threshold_THz], emission.params)[1]]; marker=:star5, markersize=18, color="#7a5c00", label="intersection")
